...
There is no test suite and no linter. After meaningful changes, run `npm run typecheck` and `npm run build`, then verify in the Electron app (see "Manual checks" in `docs/architecture.md`).
...
Per `docs/architecture.md`, the editing surface is intentionally minimal. Avoid adding tabs, command palettes, global search, secondary navigation, AI flows, or review workflows until the core editor/file-list/autosave/image experience is solid. Typography controls belong in one compact popover, not the topbar.
